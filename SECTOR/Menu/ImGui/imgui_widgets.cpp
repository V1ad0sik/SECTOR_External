...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 pading = ImVec2(2, 2);                                                   // 32                                     9
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(40, 20));
    ...
    if (*v)
    {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max - ImVec2(20, 1), ImColor(0.1f, 0.1f, 0.1f));
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1) - ImVec2(20, 1), ImColor(0, 150, 230));
    }

    else
    {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max - ImVec2(20, 1), ImColor(0.1f, 0.1f, 0.1f));
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(1, 1), check_bb.Max - ImVec2(1, 1) - ImVec2(20, 1), ImColor(30, 30, 30));
    }

    if (label_size.x > 0.0f)
        RenderText(text_bb.GetTL() - ImVec2(20, 0), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 1.5f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
